    ...
    <div className="weather-card backdrop-blur-md bg-gradient-to-br from-white/90 to-white/70 overflow-hidden">
      <div className="relative">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 relative z-10">
          ...
          <div className="space-y-6">
            {/* Восход и закат */}
            <div className="flex items-center justify-between p-4 bg-white/20 rounded-xl backdrop-blur-sm">
              <div className="flex items-center space-x-3">
                <WiSunrise className="text-3xl text-amber-500" />
                <div>
                  <p className="text-sm text-white-500">Восход</p>
                  <p className="font-semibold text-white-800">{formatTime(data.sunrise)}</p>
                </div>
              </div>
              <div className="flex items-center space-x-3">
                <WiSunset className="text-3xl text-orange-500" />
                <div>
                  <p className="text-sm text-white-500">Закат</p>
                  <p className="font-semibold text-white-800">{formatTime(data.sunset)}</p>
                </div>
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="flex items-center space-x-3 p-4 bg-white/20 rounded-xl backdrop-blur-sm transition-all hover:bg-white/70">
                <WiThermometer className="text-3xl text-blue-500" />
                <div>
                  <p className="text-sm text-white-500">Ощущается как</p>
                  <p className="font-semibold text-white-800">{Math.round(data.apparent_temperature)}°C</p>
                </div>
              </div>
              <div className="flex items-center space-x-3 p-4 bg-white/20 rounded-xl backdrop-blur-sm transition-all hover:bg-white/70">
                <WiHumidity className="text-3xl text-teal-500" />
                <div>
                  <p className="text-sm text-white-500">Влажность</p>
                  <p className="font-semibold text-white-800">{data.humidity}%</p>
                </div>
              </div>
              <div className="flex items-center space-x-3 p-4 bg-white/20 rounded-xl backdrop-blur-sm transition-all hover:bg-white/70">
                <WiStrongWind className="text-3xl text-cyan-500" />
                <div>
                  <p className="text-sm text-white-500">Ветер</p>
                  <p className="font-semibold text-white-800">{Math.round(data.wind_speed)} м/с</p>
                </div>
              </div>
              <div className="flex items-center space-x-3 p-4 bg-white/20 rounded-xl backdrop-blur-sm transition-all hover:bg-white/70">
                <WiBarometer className="text-3xl text-indigo-500" />
                <div>
                  <p className="text-sm text-white-500">Давление</p>
                  <p className="font-semibold text-white-800">{Math.round(data.pressure)} гПа</p>
                </div>
              </div>
            </div>
          </div>
